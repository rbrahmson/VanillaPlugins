...
class PorterStemmer  {
	/**
	* This code appears on the Martin Porter (the algorithm author) web site: http://tartarus.org/~martin/PorterStemmer/php.txt
	* For more see: http://tartarus.org/~martin/PorterStemmer/index.html
	* 
    ...
    */
        ...
        */
        ...
        {
			// return $word;	//  Uncomment to negate function for non-English languages
            if (strlen($word) <= 2) {
                return $word;
            }
        ...
        private static function step1ab($word)
        {
            ...
            if (substr($word, -2, 1) != 'e' OR !self::replace($word, 'eed', 'ee', 0)) { // First rule
                ...
                if (   preg_match("#$v+#", substr($word, 0, -3)) && self::replace($word, 'ing', '')
					OR preg_match("#$v+#", substr($word, 0, -4)) && self::replace($word, 'ings', '')
					OR preg_match("#$v+#", substr($word, 0, -4)) && self::replace($word, 'tion', '')
